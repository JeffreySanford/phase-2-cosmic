...
- Scaffold and implement FSP allocator simulator; unit tests.
- CI integration for simulator (`ci:integration`).
- Begin Public-data ingest work (NRAO TAP stub, seed imagery configuration).
...
- [ ] Scaffold simulator service and README.
- [ ] Implement finite FSP capacity model and allocation algorithm.
- [ ] Add contention detection and typed error responses.
- [ ] Add unit tests for contention, incompatible spectral plans, and capacity exhaustion.
- [ ] Add CI job that runs simulator unit tests.
- [ ] Scaffold simulator service and README. (backend, docker)
- [ ] Implement finite FSP capacity model and allocation algorithm. (backend)
- [ ] Add contention detection and typed error responses. (backend)
- [ ] Add unit tests for contention, incompatible spectral plans, and capacity exhaustion. (backend)
- [ ] Add CI job that runs simulator unit tests. (ci, backend)

### Sprint 3 — Execution API & Security

- [ ] Implement `POST /api/v1/execution/plans` (validate + schema/allocator checks).
- [ ] Implement `POST /api/v1/execution/plans/{id}/apply` with `Idempotency-Key` handling.
- [ ] Implement `GET /api/v1/execution/plans/{id}` for plan status/history.
- [ ] Enforce `AuthFilter` on execution endpoints; add role checks.
- [ ] Add negative-path tests for idempotency and auth.
- [ ] Implement `POST /api/v1/execution/plans` (validate + schema/allocator checks). (backend)
- [ ] Implement `POST /api/v1/execution/plans/{id}/apply` with `Idempotency-Key` handling. (backend)
- [ ] Implement `GET /api/v1/execution/plans/{id}` for plan status/history. (backend)
- [ ] Enforce `AuthFilter` on execution endpoints; add role checks. (backend)
- [ ] Add negative-path tests for idempotency and auth. (backend)

### Sprint 4 — Mode-aware Backend Orchestration

- [ ] Create job templates for VLBI, Pulsar-Timing, Pulsar-Search, Correlation modes.
- [ ] Implement mode routing service that selects and fills templates.
- [ ] Add integration tests verifying template selection and provenance fields.
- [ ] Create job templates for VLBI, Pulsar-Timing, Pulsar-Search, Correlation modes. (backend)
- [ ] Implement mode routing service that selects and fills templates. (backend)
- [ ] Add integration tests verifying template selection and provenance fields. (backend, docker)

### Sprint 5 — Cypress Remediation & Deterministic e2e

- [ ] Root-cause analysis for Cypress cache/runtime failures.
- [ ] Patch `apps/frontend-e2e` and CI `e2e.yml` to remediate caching issues.
- [ ] Re-enable `datasets-provenance.cy.ts` in smoke lane and verify stability.
- [ ] Add CI smoke job assertion for deterministic pass.
- [ ] Root-cause analysis for Cypress cache/runtime failures. (frontend)
- [ ] Patch `apps/frontend-e2e` and CI `e2e.yml` to remediate caching issues. (frontend, ci)
- [ ] Re-enable `datasets-provenance.cy.ts` in smoke lane and verify stability. (frontend, ci)
- [ ] Add CI smoke job assertion for deterministic pass. (ci)

### Sprint 6 — Job Manifest / Lineage

- [ ] Implement `GET /api/v1/jobs/{id}/lineage` and `POST/PATCH /api/v1/jobs/{id}/lineage`.
- [ ] Frontend: add lineage display to Jobs page and submission support in job dialog.
- [ ] Add unit/component tests and e2e scenario that verifies lineage round-trip.
- [ ] Implement `GET /api/v1/jobs/{id}/lineage` and `POST/PATCH /api/v1/jobs/{id}/lineage`. (backend)
- [ ] Frontend: add lineage display to Jobs page and submission support in job dialog. (frontend)
- [ ] Add unit/component tests and e2e scenario that verifies lineage round-trip. (frontend, backend, ci)

### Cross-Sprint Items

- [ ] Provenance E2E: manifest verification + `GET /api/v1/audit` endpoint.
- [ ] Broker runbook: finalize DLQ/replay safety updates and link to docs.
- [ ] Jobs UX: finalize lineage submission UX prototype and integrate behind feature flag.
- [ ] Public-data ingest: implement NRAO TAP ingest stub + seed imagery config and source attribution fields.
...
- [ ] Add `ci:integration` job to CI pipeline and document run steps.
- [ ] Add `e2e:ci:smoke` job that runs deterministic smoke scenarios used across sprints.
- [ ] Add `e2e:ci:full` nightly job and artifact publishing.
...
- [ ] Ensure local dev environment: Node toolchain (`pnpm`), Java JDK (17+), Maven, Docker/Compose, Testcontainers ready.
- [ ] Add feature-flag toggle infrastructure (if not present) to gate in-progress UI work.
- [ ] Create branch naming convention: `feature/trident/<sprint>-<short-desc>`.

### Sprint 1 — Domain Models & Schemas (concrete steps)

- [ ] Work on `main`: add JSON schema files under `openapi/schemas/trident/` for each entity. (backend)
- [ ] Implement Java records (or TS types) in `apps/java-governance` / `apps/frontend` as needed. (backend, frontend)
- [ ] Wire schemas into `SchemaService` and add resolution unit test. (backend)
- [ ] Add OpenAPI fragments and regenerate API docs; run `scripts/openapi-validate.sh` or `pnpm openapi:validate`. (backend)
- [ ] Add unit tests: serialization, required fields, negative-case invalid payloads. (backend)
- [ ] Commit changes to `main` and ensure `openapi-validate` and `ci:unit` pass. (ci)
...
Unit test scaffold added: [apps/java-governance/src/test/java/com/cosmic/governance/api/service/TridentSchemaTest.java](apps/java-governance/src/test/java/com/cosmic/governance/api/service/TridentSchemaTest.java)

Next: wire schemas into runtime `SchemaService` automatic loading (if desired), add negative-case validation tests, and add OpenAPI fragments.

### Sprint 2 — FSP Allocator Simulator (concrete steps)

- [ ] Work on `main`: scaffold simulator under `tools/trident-allocator/` with README and simple REST endpoint `/allocate`. (backend, docker)
- [ ] Implement allocation algorithm and add deterministic unit tests covering contention and failure modes. (backend)
- [ ] Add CI job entry to run simulator unit tests (`ci:integration`); configure job to start without external broker dependencies. (ci)
- [ ] Add lightweight integration test that posts a sample `SchedulingBlock` and asserts allocation result. (backend)
- [ ] Commit changes to `main` after CI `ci:integration` passes. (ci)

### Sprint 3 — Execution API & Security (concrete steps)

- [ ] Work on `main`: implement controllers: `POST /api/v1/execution/plans`, `POST /api/v1/execution/plans/{id}/apply`, `GET /api/v1/execution/plans/{id}`. (backend)
- [ ] Integrate allocator validation call into plan `validate` flow. (backend)
- [ ] Implement idempotency storage and middleware to detect duplicate `Idempotency-Key` values. (backend)
- [ ] Add AuthFilter role checks and unit/integration tests for 401/403 flows. (backend)
- [ ] Add integration tests that exercise the full validate → apply lifecycle (run under `ci:integration`). (ci, backend)
- [ ] Commit changes to `main` after CI `ci:integration` passes. (ci)

### Sprint 4 — Mode-aware Backend Orchestration (concrete steps)

- [ ] Work on `main`: author template definitions and mapping code for VLBI / Pulsar modes under `apps/java-governance/templates`. (backend)
- [ ] Implement mode router service and unit tests for template selection. (backend)
- [ ] Add integration harness that simulates backend submission and inspects the produced job template. (backend, docker)
- [ ] Commit changes to `main` after CI and integration tests pass. (ci)

### Sprint 5 — Cypress Remediation & Deterministic e2e (concrete steps)

- [ ] Work on `main`: run local reproduction: `pnpm --filter apps/frontend-e2e test` and capture failing traces. (frontend)
- [ ] Identify cache/runtime root cause (CI logs): adjust `cypress.config.ts` and `e2e.yml` to disable problematic cache options or add explicit waits where deterministic. (frontend, ci)
- [ ] Add `e2e:ci:smoke` job that runs only deterministic scenarios (e.g., `datasets-provenance.cy.ts`). (ci)
- [ ] Document `pnpm e2e:local` run steps in `apps/frontend-e2e/README.md` for contributors. (frontend)
- [ ] Commit fixes to `main` when the smoke lane is stable locally and in CI. (ci)

### Sprint 6 — Job-manifest / Lineage (concrete steps)

- [ ] Work on `main`: implement `GET /api/v1/jobs/{id}/lineage` and `POST/PATCH /api/v1/jobs/{id}/lineage` with validation. (backend)
- [ ] Frontend: add `Lineage` tab + `Parent Job` editor to `SubmitJobDialogComponent` behind feature flag. (frontend)
- [ ] Add unit, component, and e2e tests that submit a job with lineage and visualize it in Jobs list/detail. (frontend, backend, ci)
- [ ] Commit changes to `main` after tests and CI pass. (ci)

### Cross-sprint CI/Test Integration Steps

- [ ] Add `ci:integration` job to CI config that runs integration tests and simulator harness; ensure it runs in PRs or nightly as configured.
- [ ] Add `e2e:ci:smoke` job to PR pipeline for deterministic smoke tests; block merges on failure.
- [ ] Add nightly `e2e:ci:full` job and artifact publishing for failures (screenshots, videos, logs).
- [ ] Add coverage gating: fail PR if unit coverage for changed modules falls below threshold.
- [ ] Document debugging steps and reproduce commands in `documentation/development/CONTRIBUTING-E2E.md`.

### Documentation & Runbooks

- [ ] Update `BROKER_SAFETY_RUNBOOK.md` with DLQ/replay guidance linked to UI replay controls.
- [ ] Add `TRIDENT_GATEWAY.md` describing schemas, allocator, and execution API for operators.
- [ ] Add developer docs for running simulator and deterministic e2e locally.
