...
```json
{
  ...
  "sourceAttribution": {
    "sourceType": "imagery | catalog | metadata | calibration",
    "sourceName": "VLASS",
    "providerName": "NRAO",
    "sourceState": "live | fallback | cached | stale | mock",
...
For datasets derived from external/public sources, `sourceAttribution` should be treated as first-class metadata rather than UI-only decoration.
...
External/public-source quality additions:

- If `accessPolicy=PUBLIC` and the dataset derives from an external source, `sourceAttribution.citationUrl` must be present.
- `providerName` and `sourceName` should be present for externally sourced viewer-linked datasets.
- If a dataset is materialized from a public catalog/archive pull, persist retrieval/access timestamp when known.
...
- External/public-source citations must survive ingest so UI surfaces can link back to authoritative source pages.
