...
- add lower-left viewer mode control (`Auto`, `High Resolution`, `Preview`)
- implement zoom/FOV-driven progressive survey switching in `Auto`
- prefer higher-resolution public surveys for known objects where available
- show active survey and source-state labels at all times
- show compact source attribution for externally sourced imagery/overlays and preserve authoritative source URL in detail panels
- prefer public viewer seed sources first (`VLASS` HiPS/basic products, `NVAS` historical images) while deeper archive ingest is being built
- define decision gate for when a new viewer engine is required beyond Aladin capabilities

Viewer source-attribution rule:

- If a viewer surface renders external/public imagery, catalog overlays, or metadata, show source name and link as subdued small text when layout allows.
- If layout is too constrained, preserve the source citation in the selected-object, layer, or detail panel rather than dropping it.
- Citation metadata should be carried with the underlying layer/model, not assembled ad hoc in the template.
