...
Implementation decisions (locked 2026-03-03):

- Local default deployment includes Pulsar in the normal Docker stack (full profile), not as optional standalone-only.
- RabbitMQ control routing uses dynamic per-workflow queue provisioning for scalability.
- Global footer load profile control (`10%`, `25%`, `50%`, `100%`) must scale all enabled broker paths (Pulsar, Kafka, RabbitMQ) in local runtime.
- Dynamic RabbitMQ queue naming convention: `workflow.<workflowId>.commands`.
- `100%` profile is a bounded burst mode (3 minutes), then auto-reverts to `10%`.
- Stress scaling includes message rate, message size, and fanout together to emulate realistic NGVLA-class load envelopes.
...
flowchart LR
  ...
  class L_edge,L_broker,L_processing,L_storage legendClass
    L_edge[Edge]
    L_broker[Broker]
    L_processing[Processing]
    L_storage[Storage]
...
- Include default local containers for: Pulsar full profile (broker + bookkeeper + supporting services), Kafka + Zookeeper (or KRaft), RabbitMQ, a small Kafka Connect instance, and the `tools/data-generator` service.
...
- End-to-end smoke test:
  ...
  1. Run `tools/data-generator` to produce sample telemetry to PulsarEdge.
  1. Verify a message delivered to Kafka and written to the object store via connector.
  1. Verify governance control messages sent via RabbitMQ are mirrored into Kafka audit topics.
  1. Verify `100%` global burst profile stops after 3 minutes and automatically reverts to `10%`.
  1. Verify rate + payload-size + fanout all increase during burst and return to baseline after revert.

## Next steps

- Add `docker/dev-compose.yml` examples and connector configs to `documentation/` (recommended next task).
- Keep Pulsar/RabbitMQ docs and test matrix synchronized with roadmap/TODO updates.
