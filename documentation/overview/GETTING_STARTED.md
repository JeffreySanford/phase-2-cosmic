...
The local developer environment is intentionally hybrid:

- Docker Compose runs infrastructure and backend services.
- Host processes run the Nest SSR shim and Angular dev server for faster frontend iteration.
...
```bash
# Bring up hybrid infra + frontend (foreground)
pnpm start:all
...
- Check Docker services with `docker ps` to confirm Kafka, Pulsar, MinIO, Prometheus, and governance services are running.
