...
4. Timing integrity checks pass for promoted datasets (timing metadata present, drift/offset within configured budget).
5. RFI/EMC events are surfaced with quality-flag propagation and operator-visible indicators.
6. Archive DR drill demonstrates configured RPO/RTO targets and post-restore catalog/provenance consistency.
7. Interoperability gate: planned VO service interfaces (TAP/ADQL + DataLink/SODA) pass conformance checks for representative queries.
