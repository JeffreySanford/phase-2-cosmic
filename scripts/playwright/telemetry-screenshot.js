...
(async () => {
  ...
  const browser = await chromium.launch({ headless: false, slowMo: 25, args: ['--no-sandbox'] });
  ...
  page.setDefaultTimeout(120000);
    ...
    page.on('request', (req) => {
      networkLogs.push({ type: 'request', method: req.method(), url: req.url(), headers: req.headers() });
    });
    page.on('response', async (res) => {
      try {
        ...
        networkLogs.push({ type: 'response', status: res.status(), url: res.url(), headers, body });
      } catch (e) {
        networkLogs.push({ type: 'response', status: res.status(), url: res.url(), err: String(e) });
      }
    });
  ...
  try {
    ...
    await page.waitForFunction(() => {
      ...
      return !!(p && p.getAttribute && p.getAttribute('d') && p.getAttribute('d').length > 10);
    }, { timeout: 120000 });
  } catch (e) {
    console.warn('Timed out waiting for D3 line, continuing to capture whatever is present');
  }
  ...
  fs.writeFileSync('telemetry-console.json', JSON.stringify(consoleLogs, null, 2));
  ...
  try {
    ...
    console.error('Wrote telemetry-console.json. Leaving browser open for inspection.');
