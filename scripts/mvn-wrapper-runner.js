...
const isJavaGovernanceTest = args.includes('test') && fIndex !== -1 && args[fIndex + 1] && args[fIndex + 1].includes('apps/java-governance');
if (isJavaGovernanceTest || useCompose) {
  console.log('Forcing Maven in Docker for java-governance test so redis resolves on the compose network');
  // try to discover the compose network by inspecting the running redis container
  let redisContainerId = runCapture('docker', ['ps', '--filter', 'name=redis', '--format', '{{.ID}}']);
  let networkArg = null;
  if (redisContainerId) {
    const netnames = runCapture('docker', ['inspect', '-f', "{{range $k,$v := .NetworkSettings.Networks}}{{$k}} {{end}}", redisContainerId]);
    if (netnames) {
  ...
  const mvnCommand = `cd /workspace && mvn ${args.map(a => a.includes(' ') ? '"' + a.replace(/"/g, '\\"') + '"' : a).join(' ')}`;
  const dockerCmd = ['run', '--rm'];
  if (networkArg) dockerCmd.push(...networkArg);
  dockerCmd.push('-v', `${volPath}:/workspace`, '-w', '/workspace', dockerImage, 'bash', '-lc', mvnCommand);
  console.log('Running Maven in Docker image', dockerImage, 'network:', networkArg ? networkArg[1] : '(default)');
