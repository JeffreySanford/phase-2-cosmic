...
resolve_node_cli() {
  ...
  local cli_path="$2"
  local resolved_path="$REPO_ROOT/node_modules/$package_name/$cli_path"
...
CONCURRENTLY_JS="$(resolve_node_cli concurrently dist/bin/concurrently.js || true)"
TSX_CLI_JS="$(resolve_node_cli tsx dist/cli.mjs || true)"
if [ -z "$CONCURRENTLY_JS" ] || [ -z "$TSX_CLI_JS" ]; then
  log "[start-all-reset] Missing local dev dependencies. Run 'pnpm install' in $REPO_ROOT and retry."
  exit 1
fi
...
if command -v cygpath >/dev/null 2>&1; then
  ...
  WIN_TSX_CLI_JS="$(cygpath -w "$TSX_CLI_JS")"
else
  WIN_REPO_ROOT="$REPO_ROOT"
  WIN_TSX_CLI_JS="$TSX_CLI_JS"
fi
