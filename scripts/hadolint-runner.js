...
function findDockerfiles() {
  ...
  try {
    ...
    const out = execSync("git ls-files -- '*/Dockerfile' || true", { encoding: 'utf8' }).trim();
...
for (const f of files) {
  ...
  const args = ['run', '--rm', '-v', `${process.cwd()}:/workdir`, '-w', '/workdir', 'hadolint/hadolint', 'hadolint', f.replace(/\\/g, '/')];
  ...
  const argsPosix = ['run', '--rm', '-v', `${process.cwd()}:/workdir`, '-w', '/workdir', 'hadolint/hadolint', 'hadolint', rel];
