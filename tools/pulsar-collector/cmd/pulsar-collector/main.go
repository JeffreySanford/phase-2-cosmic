...
func run(cfg Config) error {
	...
	for {
		...
		kerr := writer.WriteMessages(ctx, kafka.Message{
			Value:   msg.Payload(),
			Headers: attributionHeaders(cfg.Region, msg),
		})
...
// mutating the payload, keeping the forwarded record source-faithful.
func attributionHeaders(region string, msg pulsar.Message) []kafka.Header {
	return []kafka.Header{
