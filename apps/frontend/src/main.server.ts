...
const bootstrap = async (): Promise<ApplicationRef> => {
  ...
  const resolver = async (url: string) => {
    ...
    candidates.push(
      join(process.cwd(), "apps", "frontend", "src", url.replace(/^\.\//, ""))
    );
    candidates.push(join(process.cwd(), url));
    ...
    try {
      ...
      const origin =
        process.env["DEV_SERVER_ORIGIN"] || "http://localhost:4200";
      ...
      try {
        console.debug(
          `[SSR resolver] fetch fallback failed for: ${url} - ${String(e)}`
        );
      } catch {}
    }
  ...
  try {
    ...
    const resolverFn =
      ngCore.resolveComponentResources || ngCore["ɵresolveComponentResources"];
    console.log(
      `[SSR resolver] using resolver fn: ${resolverFn ? "present" : "missing"}`
    );
    if (resolverFn) {
