...
import {
  ...
  ProvenanceInfo,
} from "./provenance-panel.component";
...
describe("ProvenancePanelComponent", () => {
  ...
  describe("ngVLA parameters", () => {
    ...
    it("should display arraySegment badge", () => {
      ...
      const arraySegmentBadge = Array.from(badges).find((b) =>
    ...
    it("should display antennaClass badge", () => {
      const compiled = fixture.nativeElement as HTMLElement;
      const badges = compiled.querySelectorAll(".provenance-panel__badge");
      const antennaClassBadge = Array.from(badges).find((b) =>
        b.textContent?.includes("18m")
      );
  ...
  describe("toggle functionality", () => {
    ...
    it("should toggle chevron icon class", () => {
      ...
      expect(
        chevron?.classList.contains("provenance-panel__chevron--expanded")
      ).toBe(false);
