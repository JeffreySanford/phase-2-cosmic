...
@Component({
  ...
  styles: [`
    ...
    .status-band--healthy {
      background: #e8f5e9;
      color: #2e7d32;
      display: none; /* Hide when system is healthy */
    }
...
export class StatusBandComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subscription = this.statusService.status$.subscribe(status => {
      this.status = status;
      // Only show band when there's a problem (degraded or offline)
      this.shouldShow = status.health !== 'healthy';
    });
  }
  ...
  getMessage(): string {
    ...
    switch (this.status.health) {
      ...
      case 'offline': return 'System offline - check connection';
