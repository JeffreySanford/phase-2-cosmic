...
describe("NGVLA Reference Drift Tests", () => {
  ...
  interface NgvlaFixture {
    arrayConfiguration: {
      ...
      antennaClass: string;
      baselineRange: { minimum: number; maximum: number };
  ...
  beforeAll(() => {
    ...
    longBaselineFixture = JSON.parse(
      fs.readFileSync(
        path.join(fixturesDir, "ngvla-long-baseline.json"),
        "utf-8"
      )
    );
    shortBaselineFixture = JSON.parse(
      fs.readFileSync(
        path.join(fixturesDir, "ngvla-short-baseline.json"),
        "utf-8"
      )
    );
  });
  ...
  describe("Main Array Configuration", () => {
    ...
    it("should have correct baseline range (26m to 1,005km)", () => {
      expect(mainArrayFixture.arrayConfiguration.baselineRange.minimum).toBe(
        26
      );
    ...
    it("should have all 6 frequency bands defined", () => {
      ...
      const bandNames = mainArrayFixture.frequencyBands.map(
        (b: FrequencyBand) => b.band
      );
    ...
    it("should have correct Band 1 frequency range (1.2-3.5 GHz)", () => {
      const band1 = mainArrayFixture.frequencyBands.find(
        (b: FrequencyBand) => b.band === "Band 1"
      );
    ...
    it("should have correct Band 6 frequency range (50-116 GHz)", () => {
      const band6 = mainArrayFixture.frequencyBands.find(
        (b: FrequencyBand) => b.band === "Band 6"
      );
  ...
  describe("Long Baseline Array Configuration", () => {
    ...
    it("should have correct baseline range (1,000km to 8,946km)", () => {
      expect(longBaselineFixture.arrayConfiguration.baselineRange.minimum).toBe(
        1000000
      );
      expect(longBaselineFixture.arrayConfiguration.baselineRange.maximum).toBe(
        8946000
      );
    });

    it('should have canonical label "Long Baseline"', () => {
      expect(longBaselineFixture.arrayConfiguration.canonicalLabel).toBe(
        "Long Baseline"
      );
    ...
    it("should have high-frequency bands only (Band 5, Band 6)", () => {
      expect(longBaselineFixture.frequencyBands.length).toBeGreaterThanOrEqual(
        2
      );
      const bandNames = longBaselineFixture.frequencyBands.map(
        (b: FrequencyBand) => b.band
      );
  ...
  describe("Short Baseline Array Configuration", () => {
    ...
    it("should have correct baseline range (9m to 330m)", () => {
      expect(
        shortBaselineFixture.arrayConfiguration.baselineRange.minimum
      ).toBe(9);
      expect(
        shortBaselineFixture.arrayConfiguration.baselineRange.maximum
      ).toBe(330);
    });

    it('should have canonical label "SBA"', () => {
      expect(shortBaselineFixture.arrayConfiguration.canonicalLabel).toBe(
        "SBA"
      );
    ...
    it("should have low-to-mid frequency bands (Band 1-4)", () => {
      expect(shortBaselineFixture.frequencyBands.length).toBeGreaterThanOrEqual(
        4
      );
      const bandNames = shortBaselineFixture.frequencyBands.map(
        (b: FrequencyBand) => b.band
      );
  ...
  describe("Frequency Band Consistency", () => {
    it("should have consistent Band 1 range across applicable arrays (1.2-3.5 GHz)", () => {
      const mainBand1 = mainArrayFixture.frequencyBands.find(
        (b: FrequencyBand) => b.band === "Band 1"
      );
      const sbaBand1 = shortBaselineFixture.frequencyBands.find(
        (b: FrequencyBand) => b.band === "Band 1"
      );
      if (!mainBand1 || !sbaBand1)
        fail("Band 1 missing in one of the fixtures");
      expect(mainBand1.frequencyRangeGHz.min).toBe(
        sbaBand1.frequencyRangeGHz.min
      );
      expect(mainBand1.frequencyRangeGHz.max).toBe(
        sbaBand1.frequencyRangeGHz.max
      );
    });
    ...
    it("should have valid reference URLs", () => {
      expect(mainArrayFixture.reference.url).toMatch(
        /^https:\/\/library\.nrao\.edu/
      );
      expect(longBaselineFixture.reference.url).toMatch(
        /^https:\/\/library\.nrao\.edu/
      );
      expect(shortBaselineFixture.reference.url).toMatch(
        /^https:\/\/library\.nrao\.edu/
      );
    });
  });
