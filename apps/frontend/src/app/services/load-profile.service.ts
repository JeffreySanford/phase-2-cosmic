...
export class LoadProfileService {
  ...
  private modeSubject = new BehaviorSubject<'baseline' | 'runtime-controlled'>('baseline');
  ...
  setProfile(pct: LoadProfilePct): void {
    this.http.post<{ profilePct: LoadProfilePct; mode?: 'baseline' | 'runtime-controlled' }>('/api/load-profile', { profilePct: pct }).subscribe({
      next: (resp) => {
        const nextPct = (resp?.profilePct ?? pct) as LoadProfilePct;
        this.profileSubject.next(nextPct);
        this.modeSubject.next(resp?.mode ?? (nextPct === 10 ? 'baseline' : 'runtime-controlled'));
        this.persist(nextPct);
      },
      error: (err) => {
        console.warn('Failed to apply runtime load profile, keeping local profile state:', err);
  ...
  private refreshRuntimeStatus(): void {
    this.http.get<{ profilePct?: LoadProfilePct; mode?: 'baseline' | 'runtime-controlled' }>('/api/load-profile').subscribe({
      next: (resp) => {
