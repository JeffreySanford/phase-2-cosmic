...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  healthScore = 0;
  systemEnergy = 0;
  signalBands: Array<{ label: string; value: number; tone: 'mint' | 'violet' | 'amber' | 'rose' }> = [];
  ...
  refresh(): void {
    ...
    const cpuLoad$ = this.probe(
      this.telemetry.queryInstant('100 * sum(rate(process_cpu_seconds_total{job=~"data-generator|java-ingest"}[1m]))'),
      0
    );
    const targetsUp$ = this.probe(this.telemetry.queryInstant('sum(up)'), 0);
    const generatorUp$ = this.probe(this.telemetry.queryInstant('up{job="data-generator"}'), 0);
    ...
    this.sub.add(
      forkJoin({
        ...
        const files = (snapshot.diagnostics.value.files || []).filter((f) => f !== '.gitkeep');
        ...
        const targetsSeries = this.extractRangeValues(snapshot.targetsRange.value);
        ...
        const ingestIntensity = this.normalize(snapshot.bytesRate.value, 0, 1024 * 300);
        const recordsIntensity = this.normalize(snapshot.recordsRate.value, 0, 800);
        const totalIntensity = this.normalize(snapshot.totalBytes.value, 0, 1024 * 1024 * 1024);
        this.telemetryTiles = [
          {
          ...
          },
          {
            ...
            value: `${Math.max(0, snapshot.recordsRate.value).toFixed(2)} rec/s`,
          ...
          {
            title: 'Total Bytes',
            value: new Intl.NumberFormat().format(Math.round(Math.max(0, snapshot.totalBytes.value))),
        ...
        this.generatorStatus = {
          ...
          lastSeen: snapshot.generatorUp.value >= 1 ? new Date().toLocaleTimeString() : 'n/a',
          source: snapshot.generatorUp.ok && snapshot.targetsUp.ok ? 'live' : 'fallback',
        };

        this.alerts = this.buildAlerts(snapshot.cpuLoad.value, snapshot.bytesRate.value, snapshot.generatorUp.value, files.length);
        ...
        this.systemEnergy = this.clamp(
          Math.round(this.normalize(this.bytesRateValue, 0, 1024 * 300) * 62 + this.normalize(this.recordsRateValue, 0, 900) * 38),
          0,
          100
        );
        this.signalBands = [
          { label: 'Telemetry Throughput', value: this.clamp(Math.round(ingestIntensity * 100), 0, 100), tone: 'violet' },
          { label: 'Pipeline Velocity', value: this.clamp(Math.round(recordsIntensity * 100), 0, 100), tone: 'mint' },
          { label: 'System Headroom', value: this.clamp(Math.round(100 - this.cpuLoadPct), 0, 100), tone: 'amber' },
          { label: 'Operational Health', value: this.healthScore, tone: 'rose' },
        ];
        const criticals = this.alerts.filter((a) => a.level === 'critical').length;
        const warnings = this.alerts.filter((a) => a.level === 'warning').length;
        this.headerKpis = [
          { label: 'CPU Load', value: `${this.cpuLoadPct.toFixed(0)}%`, tone: this.cpuTone(this.cpuLoadPct) },
          { label: 'Ingest Rate', value: this.formatBytesPerSecond(this.bytesRateValue), tone: 'violet' },
          { label: 'Health Score', value: `${this.healthScore}%`, tone: this.healthScore >= 70 ? 'mint' : 'amber' },
          { label: 'Alerts', value: `${criticals}/${warnings}`, tone: criticals > 0 ? 'rose' : warnings > 0 ? 'amber' : 'mint' },
        ];
  ...
  private buildAlerts(cpuLoad: number, bytesRate: number, generatorUp: number, artifactCount: number): AlertItem[] {
    const items: AlertItem[] = [];
    if (generatorUp < 1) {
      items.push({ level: 'critical', message: 'Generator scrape is down (up{job="data-generator"} < 1).' });
    }
    if (cpuLoad > 85) {
      items.push({ level: 'warning', message: `System CPU load is high at ${cpuLoad.toFixed(2)}%.` });
    }
    if (bytesRate < 1024) {
      items.push({ level: 'warning', message: 'Throughput is low (< 1 KB/s). Verify generator rate and sinks.' });
    }
    if (artifactCount === 0) {
      items.push({ level: 'info', message: 'No diagnostics artifacts found yet. Run passive diagnostics.' });
    }
    if (items.length === 0) {
      items.push({ level: 'info', message: 'No active warnings. Core telemetry signals are healthy.' });
    }
    return items;
  }
  ...
  ringStyle(value: number): string {
    ...
    return `conic-gradient(from -90deg, #00d4ff 0 ${pct * 0.5}%, #6b7aff ${pct * 0.8}%, #19d3a2 ${pct}%, rgba(255,255,255,0.08) ${pct}% 100%)`;
  }
