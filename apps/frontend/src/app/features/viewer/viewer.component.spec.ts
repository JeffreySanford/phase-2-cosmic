jest.mock(
  ...
  () => {
    const factory = jest.fn((el: HTMLElement, opts?: unknown) => {
      ...
      try {
        const child =
          el && typeof el.appendChild === "function"
            ? (el.ownerDocument || document).createElement("div")
            : null;
        if (child) {
...
describe("ViewerComponent", () => {
    ...
    return firstValueFrom(timer(50)).then(() => {
      ...
      if (typeof calledWith === "string") {
        ...
        expect(el.tagName?.toLowerCase()).toBe(
          component.containerRef.nativeElement.tagName.toLowerCase()
        );
        expect(
          el.id === component.containerRef.nativeElement.id ||
            el.classList.contains("aladin-container")
        ).toBe(true);
      }
    });
  });
  ...
  it("enables controls (addControl or setOptions) when inputs request them", () => {
    ...
    return firstValueFrom(timer(50)).then(() => {
      ...
      const setOptionsCalled =
        inst.setOptions && inst.setOptions.mock.calls.length > 0;
      const addControlCalled =
        inst.addControl && inst.addControl.mock.calls.length > 0;
