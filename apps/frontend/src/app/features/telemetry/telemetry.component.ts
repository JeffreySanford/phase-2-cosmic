...
export class TelemetryComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private renderLine(points: Array<{ t: number; v: number }>, ma: Array<{ t: number; v: number }>) {
    ...
    const yMax = maxV + pad;
    const y = d3.scaleLinear().range([h - margin.bottom, margin.top]).domain([yMin, yMax]);
    const lineGen = d3
      .line()
    ...
    this.svg.select('path.line').datum(points).attr('d', lineGen(points) ?? '');
    this.svg.select('path.ma').datum(ma).attr('d', maLineGen(ma) ?? '');
    this.renderGauge(this.currentRate, this.gaugeCap);
    ...
    const yAxis = d3.axisLeft(y).ticks(5).tickFormat((d: number) => this.humanRate(Number(d)));
    ...
    overlay.on('mousemove', (event: PointerEvent) => {
      ...
      const labelY = Math.max(margin.top + 12, Math.min(fy - 8, h - margin.bottom - 4));
      focus
  ...
  private renderGauge(value: number, cap: number) {
    ...
    const pct = Math.min(100, Math.max(0, (Math.max(0, value) / capNorm) * 100));
    this.gaugeGroup
  ...
  private renderHistogram(values: number[]) {
    ...
    const x = d3.scaleLinear().domain([0, (d3.max(data) ?? 1) * 1.05]).range([margin.left, w - margin.right]);
    ...
    const g = this.histSvg.selectAll('g.bin').data(bins, (d: unknown) => `${(d as Bin).x0}-${(d as Bin).x1}`);
    ...
    this.histSvg
      ...
      .call(d3.axisBottom(x).ticks(5).tickFormat((d: number) => this.humanRate(Number(d))));
  }
  ...
  private updateRecentSamples(points: Array<{ t: number; v: number }>) {
    ...
    this.recentSamples = points.slice(-12).reverse().map((p) => ({
