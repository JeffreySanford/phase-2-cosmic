describe('Jobs view diagnostic', () => {
  ...
  it('captures /api/v1/jobs requests and responses', () => {
    ...
    cy.document().then((d) => {
      ...
      const pageLog = 'logs/jobs-page-' + new Date().toISOString().replace(/[:.]/g,'') + '.html'
      ...
      const looksLikeGrafana = title.includes('grafana') || html.includes('window.grafanaBootData')
      ...
      if (looksLikeGrafana || !hasAppRoot) {
        ...
        cy.writeFile(diagnosticLog, JSON.stringify({ capturedAt: new Date().toISOString(), failure: failDiag }, null, 2))
        throw new Error('Non-frontend page detected; wrote ' + diagnosticLog + ' and ' + pageLog + ' — reason=' + failDiag.reason)
      }
    })
