...
test.describe("Provenance Panel", () => {
  ...
  test.describe("with provenance data", () => {
    ...
    test("should expand provenance panel when header clicked", async ({
      page,
    }) => {
      await page.goto(`${baseUrl}/datasets?e2e=1`);
      ...
      if ((await panelHeader.count()) > 0) {
        ...
        const contentAfterClick = page
          .locator(".provenance-panel__content")
          .first();
        await expect(contentAfterClick).toBeVisible();
      }
    });

    test("should display workflow information when available", async ({
      page,
    }) => {
      await page.goto(`${baseUrl}/datasets?e2e=1`);
      ...
      const workflowLabel = page.locator(
        '.provenance-panel__label:has-text("Workflow")'
      );
      ...
      if ((await workflowLabel.count()) > 0) {
        ...
        const workflowValue = page
          .locator(".provenance-panel__value code")
          .first();
        await expect(workflowValue).toBeVisible();
      }
    });

    test("should display jobId as clickable link when available", async ({
      page,
    }) => {
      await page.goto(`${baseUrl}/datasets?e2e=1`);
      ...
      const jobIdLabel = page.locator(
        '.provenance-panel__label:has-text("Job ID")'
      );
    ...
    test("should display ngVLA parameters when available", async ({ page }) => {
      ...
      const ngvlaSection = page.locator(
        '.provenance-panel__section:has-text("ngVLA Observation Parameters")'
      );
    ...
    test("should display reproducible science message in footer", async ({
      page,
    }) => {
      await page.goto(`${baseUrl}/datasets?e2e=1`);
      ...
      if ((await panelHeader.count()) > 0) {
        ...
        const footer = page.locator(
          ".provenance-panel__footer .provenance-panel__note"
        );
  ...
  test.describe("dataset creation with provenance", () => {
    ...
    test("should create dataset and display provenance panel", async ({
      ...
      const nameInput = page
        .locator('input[name="name"], mat-form-field:has-text("Name") input')
        .first();
      const descInput = page
        .locator(
          'input[name="description"], mat-form-field:has-text("Description") input'
        )
        .first();
