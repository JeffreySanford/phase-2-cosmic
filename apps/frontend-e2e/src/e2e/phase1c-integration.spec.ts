...
test.describe('Phase 1C Integration', () => {
  ...
  test.describe('NGVLA Topology Visualization', () => {
    test('should display ngVLA array segments in topology', async ({ page }) => {
      await page.goto(`${baseUrl}/topology?e2e=1`);
      
      await page.waitForSelector('.topology-container, svg', { timeout: 10000 });
    ...
    test('should display disclaimer banner on topology page', async ({ page }) => {
      await page.goto(`${baseUrl}/topology?e2e=1`);
  ...
  test.describe('Diagnostics with Demo Disclaimer', () => {
    test('should display diagnostics page with disclaimer', async ({ page }) => {
  ...
  test.describe('Cross-Page Navigation', () => {
    test('should maintain disclaimer across page navigation', async ({ page }) => {
      // Start on Jobs page
      await page.goto(`${baseUrl}/jobs?e2e=1`);
      await page.waitForSelector('app-disclaimer-banner', { timeout: 10000 });
      ...
      const datasetsLink = page.locator('a[href*="datasets"], button:has-text("Datasets")');
      if (await datasetsLink.count() > 0) {
        await datasetsLink.first().click();
        await page.waitForSelector('mat-card-title:has-text("Datasets")', { timeout: 10000 });
  ...
  test.describe('Phase 1C Exit Criteria Validation', () => {
    test('should validate all demo-facing pages have disclaimers', async ({ page }) => {
      const pages = [
        { path: '/jobs', name: 'Jobs' },
      ...
      for (const pg of pages) {
        ...
        await page.waitForSelector('app-disclaimer-banner, mat-card, h1, h2', { timeout: 10000 });
    ...
    test('should validate provenance panel integration', async ({ page }) => {
      ...
      const hasProvenancePanelComponent = bodyHtml.includes('app-provenance-panel') || 
